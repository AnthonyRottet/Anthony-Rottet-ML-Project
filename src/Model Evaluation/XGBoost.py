...
PRED_PATH = Path("../../results/XGBoost/xgboost_test_predictions.csv")
...
def main():
    if not PRED_PATH.exists():
        print(f"Error: {PRED_PATH} not found. Please run the XGBoost script first!")
        return
    ...
    # Using 'Oranges' cmap to match XGBoost's brand identity
    ...
    plt.savefig(RESULTS_DIR / "xgb_confusion_matrix.png", dpi=300)
    plt.close()
    ...
    plt.figure(figsize=(8, 6))
    plt.plot(fpr, tpr, color='darkorange', lw=2, label=f'XGB ROC curve (area = {roc_auc:.4f})')
    ...
    plt.title('ROC Curve: XGBoost Performance', fontsize=14)
    plt.legend(loc="lower right")
    plt.grid(alpha=0.3)

    plt.savefig(RESULTS_DIR / "xgb_roc_curve.png", dpi=300)
    plt.close()
