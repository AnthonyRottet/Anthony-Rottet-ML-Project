...
PARAMS_OUT = RESULTS_DIR / "logit_hyperparameters.csv"
...
def main():
    ...
    # Robust Parameter Grid (Comprehensive Multivariate Search)
    param_grid = [
        {'model__solver': ['lbfgs', 'newton-cg'], 'model__penalty': ['l2', None],
         'model__C': np.logspace(-4, 4, 10), 'model__max_iter': [2500]},
        {'model__solver': ['liblinear'], 'model__penalty': ['l1', 'l2'],
         'model__C': np.logspace(-4, 4, 10), 'model__max_iter': [2500]},
        {'model__solver': ['saga'], 'model__penalty': ['l1', 'l2', 'elasticnet', None],
         'model__C': np.logspace(-4, 4, 10), 'model__l1_ratio': [0.5], 'model__max_iter': [2500]}
    ]

    # Grid Search with CV=5 (Increasing CV to 5 for consistency with KNN/SVM)
    clf = GridSearchCV(
    ...
    # 5. EXECUTE SEARCH
    print("\n=== INITIATING LOGIT GRID SEARCH (CV=5) ===")
    print("-" * 45)
